    ...
    public async Task OnTrackProgressAsync(TimeSpan progress, TimeSpan duration)
    {
        ...
        if (isLongEnough && hasPlayedEnough)
        {
            ...
            try
            {
                ...
                {
                    Debug.WriteLine(
                        $"[LastFmPresenceService] Successfully scrobbled track in real-time: {_currentSong.Title}");
            ...
            catch (Exception ex)
            {
                ...
                Debug.WriteLine(
                    $"[LastFmPresenceService] Real-time scrobble failed for '{_currentSong.Title}'. It will be handled by the background service. Error: {ex.Message}");
            }
        }
    }
