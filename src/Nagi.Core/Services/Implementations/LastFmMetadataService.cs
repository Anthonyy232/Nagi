...
public class LastFmMetadataService : ILastFmMetadataService {
    ...
    public LastFmMetadataService(IHttpClientFactory httpClientFactory, IApiKeyService apiKeyService, ILogger<LastFmMetadataService> logger) {
        ...
        for (var attempt = 0; attempt <= MaxRetries; attempt++) {
            ...
            try {
                ...
                if (errorResponse?.ErrorCode == InvalidApiKeyErrorCode && attempt < MaxRetries) {
                    _logger.LogWarning("Last.fm API key is invalid. Refreshing and retrying request for artist '{ArtistName}'.", artistName);
                    apiKey = await _apiKeyService.RefreshApiKeyAsync(ApiKeyName, cancellationToken);
                    if (string.IsNullOrEmpty(apiKey)) {
                        _logger.LogError("Failed to refresh Last.fm API key. Aborting metadata fetch for '{ArtistName}'.", artistName);
