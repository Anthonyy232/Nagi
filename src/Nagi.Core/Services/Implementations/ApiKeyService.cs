...
public class ApiKeyService : IApiKeyService, IDisposable {
    ...
    public ApiKeyService(IHttpClientFactory httpClientFactory, IConfiguration configuration, ILogger<ApiKeyService> logger) {
    ...
    private async Task<string?> FetchKeyFromServerAsync(string keyName, CancellationToken cancellationToken) {
        ...
        try {
            ...
            if (string.IsNullOrWhiteSpace(finalKey)) {
                _logger.LogError("API key response for '{ApiKeyName}' is invalid. It is missing the 'Value' field or is empty.", keyName);
                return null;
            }
