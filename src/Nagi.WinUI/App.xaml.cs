...
namespace Nagi.WinUI {
    ...
    public partial class App : Application {
        ...
        public static Color SystemAccentColor {
            get {
                _systemAccentColor ??= Current.Resources.TryGetValue("SystemAccentColor", out object? value) && value is Color color
                    ? color
                    : Colors.SlateGray;
        ...
        private void ProcessFileActivationQueue() {
            ...
            MainDispatcherQueue?.TryEnqueue(async () => {
                ...
                try {
                    ...
                    Debug.WriteLine($"[ERROR] App.ProcessFileActivationQueue: Exception while processing queue: {ex.Message}");
                }
                finally {
        ...
        private static void InitializeDatabase(IServiceProvider services) {
            try {
                ...
                Debug.WriteLine($"[CRITICAL] App.InitializeDatabase: Failed to initialize or migrate database. {ex.Message}");
            }
        }
        ...
        private async Task SaveApplicationStateAsync(IServiceProvider services) {
            ...
            try {
                ...
                Debug.WriteLine($"[ERROR] App.SaveApplicationStateAsync: Failed to save or clear playback state. {ex.Message}");
            }
        }
        ...
        private void EnqueuePostLaunchTasks() {
            MainDispatcherQueue?.TryEnqueue(DispatcherQueuePriority.Normal, () => {
                ...
                try {
                    ...
                    Debug.WriteLine($"[ERROR] App.EnqueuePostLaunchTasks: Failed to initialize System Media Transport Controls. {ex.Message}");
                }
            });
        }
        ...
        private async Task CheckForUpdatesOnStartupAsync() {
            ...
            try {
                ...
                Debug.WriteLine($"[ERROR] App.CheckForUpdatesOnStartupAsync: Failed during startup update check. {ex.Message}");
            }
        }
    }
}