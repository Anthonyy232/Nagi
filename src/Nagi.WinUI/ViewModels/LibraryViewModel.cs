...
public partial class LibraryViewModel : SongListViewModelBase {
    ...
    protected override async Task<PagedResult<Song>> LoadSongsPagedAsync(int pageNumber, int pageSize, SongSortOrder sortOrder) => await _libraryReader.GetAllSongsPagedAsync(pageNumber, pageSize, sortOrder);
    protected override async Task<List<Guid>> LoadAllSongIdsAsync(SongSortOrder sortOrder) => await _libraryReader.GetAllSongIdsAsync(sortOrder);
