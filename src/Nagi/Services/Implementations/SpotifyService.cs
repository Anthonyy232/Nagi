...
namespace Nagi.Services {
    ...
    public class SpotifyService : ISpotifyService {
        ...
        public async Task<string?> GetArtistImageUrlAsync(string artistName, CancellationToken cancellationToken = default) {
            ...
            using var request = new HttpRequestMessage(HttpMethod.Get, $"{SpotifyApiBaseUrl}search?q={Uri.EscapeDataString(artistName)}&type=artist&limit=1");
            ...
            try {
                ...
                if (!response.IsSuccessStatusCode) {
                    ...
                    Debug.WriteLine($"Spotify artist search failed. Status: {response.StatusCode}, Content: {errorContent}");
                    return null;
                }
        ...
        private async Task<string?> FetchAndCacheAccessTokenAsync(CancellationToken cancellationToken) {
            string? spotifyCredentials = await _apiKeyService.GetApiKeyAsync(ApiKeyName, cancellationToken);
            ...
            if (parts.Length != 2) {
                Debug.WriteLine($"Error: Spotify credentials for '{ApiKeyName}' are not in the expected 'ClientId:ClientSecret' format.");
                return null;
            }
            ...
            try {
                ...
                if (!response.IsSuccessStatusCode) {
                    var errorContent = await response.Content.ReadAsStringAsync(cancellationToken);
                    Debug.WriteLine($"Error fetching Spotify access token. Status: {response.StatusCode}, Content: {errorContent}");
                    return null;
                }
                ...
                if (root.TryGetProperty("access_token", out var accessTokenElement) &&
                    ...
                    int expiresInSeconds = expiresInElement.GetInt32();
