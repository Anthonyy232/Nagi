...
public class LastFmAuthService : ILastFmAuthService {
    ...
    public async Task<(string Token, string AuthUrl)?> GetAuthenticationTokenAsync() {
        ...
        try {
            ...
            if (!response.IsSuccessStatusCode) {
                Debug.WriteLine($"[ERROR] LastFmAuthService: Failed to get token. Status: {response.StatusCode}, Response: {content}");
                return null;
            }
    ...
    public async Task<(string Username, string SessionKey)?> GetSessionAsync(string token) {
        ...
        try {
            ...
            if (!response.IsSuccessStatusCode) {
                Debug.WriteLine($"[ERROR] LastFmAuthService: Failed to get session. Status: {response.StatusCode}, Response: {content}");
                return null;
            }
            ...
            if (session != null && !string.IsNullOrEmpty(session.Key) && !string.IsNullOrEmpty(session.Name)) {
                ...
                Debug.WriteLine("[ERROR] LastFmAuthService: Failed to deserialize or extract session details from the response.");
                return null;
            }
        }
        catch (Exception ex) {
    ...
    private class LastFmTokenResponse { public string? Token { get; set; } }
    private class LastFmSessionResponse { public LastFmSession? Session { get; set; } }
