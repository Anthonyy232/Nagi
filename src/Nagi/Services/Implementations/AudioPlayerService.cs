...
public class AudioPlayerService : IAudioPlayer
{
    ...
    public AudioPlayerService(DispatcherQueue dispatcherQueue)
    {
    ...
    public async Task LoadAsync(Song song)
    {
        ...
        catch (Exception ex)
        {
            ...
            Debug.WriteLine($"[AudioPlayerService] {errorMessage}");
    ...
    private async Task UpdateSmtcDisplayAsync()
    {
        ...
        if (!string.IsNullOrEmpty(_currentSong.AlbumArtUriFromTrack))
            ...
            {
                Debug.WriteLine($"[AudioPlayerService] Error setting SMTC thumbnail: {ex.Message}");
